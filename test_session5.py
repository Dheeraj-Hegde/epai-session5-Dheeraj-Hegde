...
def test_polygon_area_invalid_sides():
    with pytest.raises(ValueError):
        session5.polygon_area(15, sides = 2), 'Minimum Trigonal is needed to calculate area!'
...
def test_speed_converter_invalid_type_time():
    with pytest.raises(ValueError):
        session5.speed_converter(50, dist='km', time=9), 'Invalid time type'
...
def test_speed_converter_invalid_value_dist():
    with pytest.raises(ValueError):
        session5.speed_converter(60, dist='miles', time='min'), 'Invalid distnace unit'

def test_speed_converter_invalid_value_time():
    with pytest.raises(ValueError):
        session5.speed_converter(60, dist='km', time='hour'), 'Invalid time unit'
